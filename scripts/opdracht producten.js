const setup = () => {
    console.log ("setup");
    document.getElementsByClassName('btnHerbereken')[0].addEventListener("click", update);
...
const update = () => {
    ...
    for (let i = 0; i < txtBtws.length; i++) {
        ...
        let subtotaal=prijs+(1+(btw/100));

        totaal+=subtotaal;


        txtSubtotalen[i].textContent=subtotaal.toFixed(2);
    }


    txtTotaal.textContent=totaal.toFixed(2);
};
